...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "manytomany-ear.ear");

        // Current directory is the root of the "manytomany-web" project. Go up one level
        // and enter the "manytomany-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../manytomany-ejb/target/manytomany-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "manytomany-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/manytomany-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "manytomany-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
